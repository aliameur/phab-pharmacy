import { getProductsByCategoryHandle } from './get-products-by-category-handle';
...
describe('getProductsByCategoryHandle', () => {
  ...
  it('fetches products successfully', async () => {
    // Mock the two calls consecutively
    mockRequestFn.mockImplementationOnce(() =>
      Promise.resolve({ body: { product_categories: [{ id: 'pcat_pants' }] } }),
    );

    mockRequestFn.mockImplementationOnce(() =>
      Promise.resolve({ body: { products: mockProducts } }),
    );

    const result = await getProductsByCategoryHandle(mockRequestFn)('pants');
  ...
  it('handles errors', async () => {
    ...
    await expect(
      getProductsByCategoryHandle(mockRequestFn)('pants'),
