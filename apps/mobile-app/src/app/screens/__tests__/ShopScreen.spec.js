...
describe('ShopScreen', () => {
  it('renders the shop screen with a search input and buttons', () => {
    const { getByPlaceholderText, getByText, getByTestId } = render(<ShopScreen navigation={{ setOptions: () => {} }} />);
    ...
    expect(getByText('Find all your healthcare needs with ease. Just type and search below.')).toBeTruthy();
  ...
  it('updates the search text state when typing in the search input', () => {
    const { getByPlaceholderText } = render(<ShopScreen navigation={{ setOptions: () => {} }} />);
  ...
  it('calls searchSubmit when the search button is pressed', () => {
    ...
    expect(navigationMock.navigate).toHaveBeenCalledWith('Search', { search: '' });
  });
